...
Safe Reinforcement Learning (Safe RL) is a field that spans diverse approaches to keep algorithms that try to maximize a return from failing or executing risky behavior (García and Fernández, 2015). This respectively implies avoiding catastrophic events or avoiding actions that lead to highly varying returns. Hence, safety can be implemented by various constraints posed on RL agents.
